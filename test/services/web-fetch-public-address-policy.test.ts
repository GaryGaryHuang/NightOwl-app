...
function assertAddressClassification(
  ...
  for (const address of addresses) {
    assert.equal(
      isAllowedPublicWebFetchAddress(address),
      expected,
      `${label}: ${address}`
    );
  }
...
const IPV4_MAPPED_IPV6_CASES = [
  {
    label: "mapped public IPv4",
    address: "::ffff:93.184.216.34",
    expected: true
  },
  {
    label: "mapped loopback IPv4",
    address: "::ffff:127.0.0.1",
    expected: false
  },
  {
    label: "mapped documentation IPv4 in hexadecimal form",
