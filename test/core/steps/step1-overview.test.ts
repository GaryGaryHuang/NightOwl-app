...
test("Step1OverviewStep prepares the exact Step 1 prompt contract from RunContext and file metadata", () => {
  ...
  assert.equal(plan.sectionKey, "overview");
  ...
  assert.doesNotMatch(plan.prompt.userMessage, /<current_review>/u);
});
