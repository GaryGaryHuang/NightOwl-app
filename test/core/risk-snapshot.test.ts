...
test("buildRiskSnapshot returns a valid RiskSnapshot for all risk levels", async (t) => {
  await t.test("must finding at threshold → High", () => {
    const snapshot = buildRiskSnapshot([createFinding("must", 90)]);
    assert.equal(snapshot.schemaVersion, 1);
    assert.equal(snapshot.derivedRiskLevel, "High");
    assert.equal(snapshot.mustCount, 1);
    assert.equal(snapshot.niceCount, 0);
    assert.deepEqual(snapshot.acceptedFindingIds, ["must-90"]);
  ...
  await t.test("must finding below threshold → Medium", () => {
    const snapshot = buildRiskSnapshot([createFinding("must", 80)]);
    assert.equal(snapshot.derivedRiskLevel, "Medium");
    assert.equal(snapshot.mustCount, 1);
    assert.equal(snapshot.niceCount, 0);
    assert.deepEqual(snapshot.acceptedFindingIds, ["must-80"]);
  });
