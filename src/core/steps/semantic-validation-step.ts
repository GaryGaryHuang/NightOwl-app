...
const SEMANTIC_VALIDATION_INSTRUCTION = [
  ...
  "- Semantic rerun: validate only whether the current payload repaired the prior `failedGates` and `requiredCorrections`.",
  ...
  "   - `impact`: the user/system impact must be specific and proportionate to the proven evidence; the candidate's `counterEvidence` must contain substantive checks (not just assertions or restatements); priority must match the proven evidence level and required action.",
  ...
  "- Otherwise set `loopControl.action = \"accept\"`: when every candidate is `approve` or `drop`, or when there are no candidates. Never rerun an empty `perFindingResults` array.",
  "",
  ...
  "- ID source: use candidate finding IDs only in `perFindingResults[].findingId`; candidate IDs are the F IDs listed in `<candidate_ids>` and repeated in `<review_state>.candidateFindings.findings[].findingId`.",
  ...
  "- loop control: `loopControl.action` is `rerun` only when at least one candidate is `rewrite_required`, and a `rerun` payload approves no candidate; otherwise `loopControl.action` is `accept`, and an empty `perFindingResults` array is never `rerun`.",
  ...
  "",
  "Candidate correction required example - no candidate can be approved because the payload needs machine-actionable repair:",
  "{\"perFindingResults\": [{\"findingId\": \"F1\", \"decision\": \"rewrite_required\", \"failedGates\": [\"impact\"], \"requiredCorrections\": [\"Prove the concrete user/system impact or convert the candidate to missing information.\"], \"reason\": \"impact is asserted but not proven\"}], \"missingInformationItems\": [], \"loopControl\": {\"action\": \"rerun\", \"reason\": \"candidate payload must repair machine-actionable evidence gaps\"}}",
  "",
