...
const STEP6_SYSTEM_ADDITION = [
  ...
  "- Return `ValidationReportV1` with per-finding decisions, approved findings, missing information items, and loop control.",
...
const STEP6_INSTRUCTION = [
  ...
  "   - `impact`: the user/system impact must be specific and proportionate to the proven evidence; the candidate must show meaningful counter-evidence was checked; classification and severity must match the proven evidence level.",
  ...
  "   - `completeness`: the candidate must close or honestly account for the source hypotheses; unproven contract, trigger, impact, or identifier claims must become missing information rather than approved defects.",
  ...
  "   - Dropped candidates must not appear in `approvedFindings`.",
  "   - Missing information must be explicit, scoped, and tied to why reliable approval is blocked.",
  "   - If no candidates can be approved, return an empty `approvedFindings` array and preserve missing-information items when applicable.",
  "",
  ...
  "",
  "If no findings can be approved because Step 5 needs correction, return: {\"perFindingResults\": [{\"findingId\": \"F1\", \"decision\": \"rewrite_required\", \"failedGates\": [\"impact\"], \"requiredCorrections\": [\"Prove the concrete user/system impact or convert the candidate to missing information.\"], \"reason\": \"impact is asserted but not proven\"}], \"approvedFindings\": [], \"missingInformationItems\": [], \"loopControl\": {\"action\": \"rerun_step5\", \"reason\": \"Step 5 must repair machine-actionable evidence gaps\"}}",
  "",
  "If reliable approval is blocked by missing information, return: {\"perFindingResults\": [{\"findingId\": \"F1\", \"decision\": \"rewrite_required\", \"failedGates\": [\"completeness\"], \"requiredCorrections\": [\"Provide the service contract for null input handling or convert to explicit missing information.\"], \"reason\": \"required external contract is unavailable\"}], \"approvedFindings\": [], \"missingInformationItems\": [{\"description\": \"Need the service contract for null input handling.\", \"whyItMatters\": \"Without the contract the validator cannot prove expected behavior.\"}], \"loopControl\": {\"action\": \"rerun_step5\", \"reason\": \"Step 5 must address missing critical contract\"}}",
