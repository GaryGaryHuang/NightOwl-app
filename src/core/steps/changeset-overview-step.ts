...
export const CHANGESET_OVERVIEW_SYSTEM_MESSAGE = [
  ...
  "- When run-level inputs are insufficient to classify scope, cross-file boundaries, behavior changes, or test-derived expectations reliably, retrieve only the minimal repo context allowed by tool policy, then stop once the extra context no longer changes this overview.",
...
const CHANGESET_OVERVIEW_INSTRUCTION = [
  ...
  "- `userBehavior`: array of `{ statement, confidence }`. Use this only for user-visible or first-party expected behavior context. `confidence` must be `explicit` or `inferred`: use `explicit` when the behavior is stated by the user and `inferred` when changed tests or explicit code contracts establish expected behavior from observable evidence. Empty array is allowed.",
  ...
  "- `missingInformation`: array of `{ description, whyItMatters }`. Include only material missing context or open questions that would affect review focus or interpretation; do not list ordinary uncertainty or generic lack of tests. Empty array is allowed.",
