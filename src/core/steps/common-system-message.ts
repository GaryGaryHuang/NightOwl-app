...
const CONTEXT_RETRIEVAL_BLOCK = createPromptBlock("context-retrieval", [
  ...
  "- For repository inspection, prefer the narrowest built-in retrieval tool needed to complete the current step reliably.",
  ...
  "- Use `bash` only for supported source-path-scoped read-only analysis that built-in retrieval tools cannot express, such as `git diff`, `git show`, or `git grep`.",
