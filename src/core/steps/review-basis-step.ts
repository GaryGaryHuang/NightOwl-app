...
const REVIEW_BASIS_INSTRUCTION = [
  ...
  "- Use `missingInformation` only when all three of the following hold: (a) allowed local checks and permitted external retrieval cannot resolve the fact; (b) downstream validation cannot test the question locally; (c) the unresolved fact would change a ReviewBasis field, `hypothesisLedger`, a downstream Candidate Findings or Semantic Validation target, or final finding priority.",
  ...
  "- Compact means merge duplicate targets and shorten strings; do not omit a separate evidence-backed validation target solely because its expected priority is `nice_to_have`, if it has a distinct trigger, contract, reachability path, or impact.",
