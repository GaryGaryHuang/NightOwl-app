...
const STEP2_SYSTEM_ADDITION = [
  ...
  "- For each relevant dependency, describe the contract from a black-box perspective first: what it is responsible for, what goes in, what comes out, what error conditions or usage constraints are visible from available evidence, and whether the diff appears to preserve or change that boundary.",
  "- If black-box evidence is insufficient to understand a dependency's contract, behavior, error conditions, or usage constraints, inspect the dependency implementation from a white-box perspective before marking uncertainty.",
...
const STEP2_INSTRUCTION = [
  ...
  "   - If black-box evidence is insufficient to understand the contract, behavior, error conditions, or usage constraints, inspect the dependency implementation from a white-box perspective before marking uncertainty",
  ...
  "   - Treat black-box insufficiency as a reason to inspect white-box implementation details for the dependency before marking uncertainty",
  "   - Mark uncertainty only when business context, requirements, dependency implementation, source code, or tool results still cannot resolve the contract or boundary",
  "",
  ...
  "    - Contract：[I/O 規格；描述輸入、輸出、錯誤條件、使用限制；若 black-box 證據不足，先檢視 white-box 實作，仍無法確認時才標示不確定性]",
