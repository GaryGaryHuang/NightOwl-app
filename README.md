...
At the current config boundary, the app reads `repo_root/.reviewconfig.json` once per run and currently supports four fields: `confidenceThresholds` for Step 5 / Step 6 deterministic filtering, `maxConcurrentFiles` for per-file worker-pool concurrency, `mcpServers` for validated repo-local custom MCP merge into review sessions, and optional `webFetchAllowedHosts` for repo-local exact-host `web_fetch` allowlist policy. Missing config falls back to `must >= 80`, `nice >= 90`, `maxConcurrentFiles = 5`, an empty custom MCP set, and no repo-local host allowlist; invalid values for any supported field fail the run before Step 0 begins. Built-in Context7 for review sessions is currently sourced from process env (`CONTEXT7_API_KEY` when present), while repo-local custom MCP definitions and exact-host `web_fetch` policy come from `.reviewconfig.json`. `web_fetch` rollout no longer requires repo-local config, but wildcard host matching, richer URL policy, remote MCP support, and judge-session MCP injection are still deferred.
