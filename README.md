...
- Step 3 (Knowledge & Source of Truth) execution for each planned file using a repo-native-first evidence contract after Step 2 succeeds, with built-in Context7 MCP available only when genuine external knowledge gaps remain
...
The full AI review orchestration is not implemented yet. This repository now includes the Step 7 foundation on top of the Step 3 repo-native-first knowledge foundation with built-in Context7 for Step 3 only, Step 4 strategy foundation, Step 5 first-pass findings foundation, Step 6 findings-finalization foundation, repo-local `confidenceThresholds` wiring for deterministic findings filtering, repo-local `maxConcurrentFiles` wiring with bounded per-file concurrency, the skipped-file pipeline foundation for Step 1–7 exhaustion, the conservative output-failure taxonomy foundation for current output edges, the shared-output abort coordination foundation for concurrent per-file output failures, the deterministic run-level aggregate summary foundation, the deterministic review index foundation, and the completed-run artifact surface foundation; broader external knowledge rollout is still deferred. `web_fetch`, user-defined MCP wiring, all-review-session `KnowledgeSvc` rollout, rollback / retry / recovery for output failures, and the complete final rendering / export pipeline are still deferred.
...
Step 3 in the current repository is intentionally **repo-native-first with built-in Context7 support**: it converges `版本／文件參考`、`採用規則與假設`、`排除範圍` from repo-native / local evidence such as version files, lockfiles, config files, internal docs, project conventions, and necessary local git metadata, and it may use built-in Context7 MCP only when genuine knowledge gaps remain after those local sources are exhausted. This foundation still does **not** enable `web_fetch`, user-defined MCP wiring, or judge-session MCP injection.
...
Future changes will extend the current Step 0 + Step 1 + Step 2 + Step 3 + Step 4 + Step 5 + Step 6 + Step 7 plus bounded per-file concurrency, skipped-file pipeline, successful-snapshot output health assessment, output-failure taxonomy, shared-output abort coordination, run-level aggregate summary, review index, and completed-run artifact surface foundations into broader external knowledge tooling beyond the current Step 3 built-in Context7 slice, rollback / retry / recovery for output failures, and the full final review output pipeline.
