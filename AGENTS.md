...
- Using `@github/copilot-sdk@1.0.0-beta.9`. Before upgrading, re-verify: runtime imports, session lifecycle, permission hooks (`onPreToolUse`, `onPostToolUseFailure`, and `onPermissionRequest`), MCP config types, system message section names, extension permission kinds, and test behavior.
