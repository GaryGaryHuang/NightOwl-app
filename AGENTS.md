...
├── core/                Business logic
│   ├── orchestrator.ts  Flow control: Step 0 → path planning → fan-out → ReviewBasis/Steps 5–7 → finalizers
│   ├── step-runner.ts   Step execution + completion check + retry
│   ├── steps/           Strategy modules for current per-file steps (ReviewBasis, Steps 5–7)
...
| `core/orchestrator` | Flow control: Step 0 → path planning → fan-out → ReviewBasis/Steps 5–7 → finalizers | Assemble prompts or write to disk directly |
...
- **Bounded concurrency**: files are processed in parallel (default 5); within each file, ReviewBasis and Steps 5–7 run strictly in sequence.
